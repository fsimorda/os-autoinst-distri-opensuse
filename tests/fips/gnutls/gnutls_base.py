...
def validate_gnutls(self):
     ...
     if not (get_var("FIPS_ENV_MODE")):
         validate_script_output("gnutls-cli --fips140-mode 2>&1", lambda x: re.search(r'library"\\sis\\sin\\sFIPS140-[2-3]\\smode.*"', x))

     assert_scritp_run("gnutls-cli -l | grep \"Certificate types\" | grep \"CTYPE-X.509\"")
